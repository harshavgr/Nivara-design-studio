...
- Smooth page transitions on navigation, staggered entrance animations,
  and crossfade transitions on the Portfolio filter grid (via Framer Motion)
- Works fully with **no database connected** — Contact form and Portfolio
  degrade gracefully instead of breaking (see Section 2 above)
- Working "Send an Enquiry" form → saved to MongoDB once connected (`/api/contact`)
- Filterable Portfolio grid, reading from MongoDB once connected (`/api/portfolio`)
